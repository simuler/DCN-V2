...
class DygraphModel():
    ...
    def create_model(self, config):
        ...
        struct = config.get("hyper_parameters.struct")
        net_struct = config.get("hyper_parameters.net_struct")
        num_experts = config.get("hyper_parameters.num_experts")
        low_rank = config.get("hyper_parameters.low_rank")
        ...
        deepcro_model = net.DeepCroLayer(
            ...
            l2_reg_cross, is_sparse, num_experts, low_rank, struct, net_struct)  
    ...
    def train_forward(self, dy_model, metrics_list, batch_data, config):
        ...
        l2_reg = config.get("hyper_parameters.l2_reg_cross", None)
        loss = self.create_loss(pred, label) + l2_loss * l2_reg
        # update metrics
